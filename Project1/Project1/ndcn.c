...
int coins[] = { 1000 , 500 , 100 , 50, 10, 5, 1 };  // 硬幣面額
int counts[7] = { 0 };  // 各面額硬幣數量

typedef struct {
    char name[100];
    int quantity;
    int price;
    int sweetness; // 僅適用於飲品
    int temperature; // 僅適用於飲品
} OrderItem;

int main() {
    int sandwich_prices[] = { 118, 118, 103, 103 }; // 卡拉雞腿, 勁辣雞腿, 豬排, 燻雞
    char* sandwich_names[] = { "卡拉雞腿總匯", "勁辣雞腿總匯", "豬排總匯", "燻雞總匯" };

    int drink_prices[] = { 68, 48, 68, 48, 68, 48, 48, 48, 38, 73, 38, 38, 38, 38, 38 }; // 各種飲品價格
